...
3. 判断：这个justification描述的是否真的是"该轴"上的差异，还是实际上是
   按照另一个轴(如物理规格、容量等级、应用场景)才能区分的取值混进来了
4. 【典型错误示例，务必对照检查】：某维度顶层逻辑是按"电池技术路线"划分
   (取值如TOPCon、BC)，但取值列表里混入了实际上是按"外部尺寸规格"才能
   区分的条目(如"1134mm矩形组件")——这种同组取值不在同一分类轴上的情况，
   【必须】判定为flag_type="dimension_value_axis_mismatch"且severity
   必须是"blocking"，不允许因为"看起来是同一产品的常见叫法"而放行。

以下是代码启发式对本轮提案的初步扫描结果（仅供参考，不能替代你的语义判断，
但如果扫描标记了某取值而你却未提出对应objection，说明你的检查不到位）：
{heuristic_warnings}
